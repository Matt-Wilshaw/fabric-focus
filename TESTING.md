...
| 41  | Stripe webhook order confirmation email (`/checkout/wh/`)                                                                           | Production checkout created the order and showed the success page, but the Stripe `payment_intent.succeeded` webhook returned `500`, so the automatic order confirmation email was not sent. Expected: the webhook reconciles the order and sends the confirmation email after payment succeeds.                  | 1) Complete a test checkout on Heroku.<br>2) Confirm the checkout success page loads and the order appears in order history.<br>3) Check Heroku logs for `/checkout/wh/` after payment.<br>4) Before fix, Stripe webhook returns `500` and the confirmation email is not received.<br>5) After fix, rerun checkout/webhook testing. | Fixed locally      | Updated `checkout/webhook_handler.py` to support Stripe PaymentIntent payloads that expose `latest_charge` instead of embedded `charges.data[0]`, and made existing-order matching use `original_bag` + `stripe_pid` first. Added regression coverage in `checkout/tests.py`. Verified locally on 2026-05-01 with `python manage.py test checkout` (`Ran 15 tests`, `OK`) and `python manage.py check`. |
